...
class DQNManager:
	...
	def setupModel(self, render, obstacle, fileLocation=None):
		...
		if fileLocation is None:
			self.modelDQN = DQN("CnnPolicy", self.vec_env, buffer_size=100000)
		else:
			self.modelDQN = DQN.load(fileLocation)
			self.modelDQN.set_env(self.vec_env)

	# Training function
	def train(self, iterations, save_interval, logging, save_location="./dqn/"):
		if self.modelDQN is None:
		...
		if logging:
			...
			for t in tqdm(range(iterations)):
				...
				self.modelDQN.learn(total_timesteps=5000, callback=callback)

				# Saves the model at a certain interval
				if t % save_interval == 0:
					self.modelDQN.save(f"{self.defaultFile}{save_location}_{t}.zip")

			self.modelDQN.save(f"{self.defaultFile}{save_location}_finalSave.zip")
			self.plotResults()
		else:
			for t in tqdm(range(iterations)):
				# Trains the model for 5000 timesteps
				self.modelDQN.learn(total_timesteps=5000)
	...
	def run(self, steps):
		...
		for _ in range(steps):
			action, _ = self.modelDQN.predict(obs)
...
dqn_manager.setupModel(render="human", obstacle=True)
...
# save_location = String (Default: "./dqn/", the save location)
dqn_manager.train(iterations=2, save_interval=2, logging=True)
